...
public class LinkedQueueClass<T> implements QueueADT<T> { 
    ...
    public void enqueue(T newItem)  { 
        QueueNode<T> newNode; 
        newNode = new QueueNode<T>(newItem, null); //create newNode 
        //if the queue was empty, after enqueue will have one node 
        if (queueFront == null) { 
            queueFront = newNode; 
            queueRear = newNode; 
        } 
     ...
     public void dequeue() throws QueueUnderflowException { 
        ...
        //if the queue had one node, after dequeue is empty 
        if (queueFront == null) 
            queueRear = null; 
    } 
}
